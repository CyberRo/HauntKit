...
if [ -z "$NEW_VERSION" ]; then
    echo -e "${RED}[!] Uso: $0 <version>   (ej. 1.9.3)${NC}"
    exit 1
fi

# ─── Validar formato de versión (x.y.z) ───
if ! echo "$NEW_VERSION" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+$'; then
    echo -e "${RED}[!] Versión inválida '$NEW_VERSION'. Debe ser MAJOR.MINOR.PATCH (ej. 1.9.3)${NC}"
    exit 1
fi
...
while IFS= read -r file; do
    ...
    if grep -qE "(v?)$CURRENT" "$file" 2>/dev/null; then
        # Reemplaza 'v<VERSION>' (ej. v1.9.0 -> v1.9.3)
        sed -i -E "s/v${CURRENT}/v${NEW_VERSION}/g" "$file" 2>/dev/null
        # Reemplaza '<VERSION>' sin prefijo (ej. 1.9.0 -> 1.9.3), pero no el VERSION propio
